...
| `artifact_iterations` | `id` PK, `artifact_id` NOT NULL FK, `requested_by` NOT NULL FK users, `intent_text` (隐私, admin god-mode 不返 raw), `target_agent_id` NOT NULL FK users role='agent', `state` CHECK ('pending','running','completed','failed'), `created_artifact_version_id` NULL FK artifact_versions, `error_reason` NULL (复用 AL-1a #249 6 reason 枚举 + AL-4 stub `runtime_not_registered`), `created_at`, `completed_at` NULL | **Phase 3 / CV-4.1 (`schema_migrations` v=18, PR #405)**。蓝图 `canvas-vision.md §1.4-§1.5` agent iterate 数据契约。立场 ① 域隔离 (messages 表无反指列); 立场 ② CV-1 commit 单源 (commit 走 `?iteration_id=` query atomic UPDATE, 不开 `POST /iterations/:id/commit` 旁路); 立场 ③ server 不算 diff (jsdiff client 端)。**反约束**: 不挂 `cursor` 列 (RT-1 envelope cursor 拆死, IterationStateChangedFrame 9 字段 cursor 是 frame 路径); 不挂 `retry_count` 列 (failed 重试 = 新 iteration_id)。索引: `idx_iterations_artifact_id_state` + `idx_iterations_target_agent`。 |
| `user_channel_layout` | (`user_id`, `channel_id`) PK 复合, `collapsed` INTEGER NOT NULL DEFAULT 0, `position` REAL NOT NULL, `created_at`, `updated_at` | **Phase 3 / CHN-3.1 (`schema_migrations` v=19, PR #410)**。蓝图 `channel-model.md §1.4 + §3.4` 作者定义大局 + 个人偏好微调; 个人偏好两维 (collapsed + position)。pin = MIN(已有 position) - 1.0 (立场 ③ 单调小数, 不裂 pinned BOOL 双源排序)。**反约束**: 不挂 hidden / muted / pinned / is_pinned / group_id 列 (mute 走 Phase 5+, hide 留 v3+); 不挂 cursor 列 (RT-1 envelope cursor 拆死); 表无 ON DELETE CASCADE (作者删 group → 个人 layout 行 lazy GC 90d cron, 立场 ⑦)。索引: `idx_user_channel_layout_user_id` (本人 GET /me/layout 热路径)。 |
| `agent_configs` | `agent_id` PK, `schema_version` INTEGER NOT NULL, `blob` TEXT NOT NULL (JSON), `created_at`, `updated_at` | **Phase 4 / AL-2a.1 (`schema_migrations` v=20, 本 PR)**。蓝图 `agent-lifecycle.md §2.1` (用户完全自主决定 agent name/prompt/能力/model) + `plugin-protocol.md §1.4` (Borgee=SSOT 字段划界) + §1.5 (热更新分级, AL-2a 不含 BPP frame). blob 仅含 Borgee 管字段 (name / avatar / prompt / model / 能力开关 / 启用状态 / memory_ref); schema_version 单调递增 (4.1.a 并发 update 末次胜出 + 严格递增 防丢失)。**反约束**: 不挂 api_key / temperature / token_limit / retry_policy 列 (runtime-only 走 AL-2a.2 server REST API fail-closed + 4.1.c reflect scan); 不挂 cursor 列 (BPP frame agent_config_update 留 AL-2b + BPP-3 同合); 不挂 org_id (跨 org 隔离走 users.org_id 单源 CM-1 #176, 不重复持有避免双源); PK 单 agent_id (SSOT blob 整体替换 PATCH 语义, 不裂 multi-row by config_key); 表无 ON DELETE CASCADE (lazy GC v3+)。索引: `idx_agent_configs_agent_id` (PATCH/GET /api/v1/agents/:id/config 热路径)。 |
| `admin_actions` | **post-ADM-3 (v=43, PR #586)**: VIEW alias of `audit_events` (read-transparent + INSTEAD OF INSERT/UPDATE triggers route to table). Original schema preserved (`id`, `actor_id`, `target_user_id`, `action`, `metadata`, `created_at` + AL-7.1 `archived_at`); CHECK enum extended to 12 字面 (post AL-7 audit_retention_override + BPP-8 5 plugin lifecycle events). | **Phase 4 / ADM-2.1 (`schema_migrations` v=22, PR #470) → ADM-3 (v=43, PR #586) RENAME → `audit_events` 表; alias view 留 backward compat 至 Phase 5+ deprecation.** 蓝图 `admin-model.md §1.4` "谁能看到什么" + §2 "Audit 100% 留痕" + §1.4 三红线 (受影响者必感知 + admin 之间互留痕)。立场承袭 ADM-2.1 立场 ①+⑤+⑥ 全 byte-identical (alias view 不破)。**反约束** (ADM-3 立场扩展 §1.3 红线): forward-only 不可 DELETE/UPDATE on `audit_events` (sweeper archive 走 `archived_at` 列 AL-7.1); admin 仍走 `/admin-api/*` god-mode endpoint (反向 grep `/admin-api.*audit_events` 0 hit); INSERT into view routed by `admin_actions_insert` INSTEAD OF trigger (gorm `(AdminAction).TableName() == "admin_actions"` 战马原代码 0 改). |
| `audit_events` | (post-ADM-3 RENAME 自 admin_actions, v=43, PR #586): `id` PK (UUID), `actor_id` NOT NULL, `target_user_id` NOT NULL, `action` TEXT NOT NULL CHECK 12 字面 byte-identical (ADM-2.1 5 admin 字面 + AP-2 'permission_expired' + BPP-8 5 plugin lifecycle 'plugin_connected/disconnected/reconnected/cold_started/heartbeat_timeout' + AL-7 'audit_retention_override'), `metadata` TEXT NOT NULL DEFAULT '' (JSON 上下文), `created_at` Unix ms, `archived_at` Unix ms NULL (AL-7.1 sparse idx WHERE archived_at IS NOT NULL — sweeper soft-archive 14d retention) | **Phase 4 / ADM-3 (`schema_migrations` v=43, PR #586)**。蓝图 `admin-model.md §1.3` 红线扩展 (ADM-3 立场 ③ "audit-forward-only 单表 = 全 actor type / forward-only 不可 DELETE/UPDATE / admin 仍走 /admin-api/* 单独路径")。**RENAME 元数据 0 数据迁移** — SQLite RENAME admin_actions → audit_events 索引/sparse idx_archived_at AL-7.1/CHECK enum 全自动跟随。**立场承袭** audit-forward-only 锁链跨 11 处: ADM-2.1+AP-2+BPP-4+BPP-7+BPP-8+HB-3 v2+AL-7+AL-8+HB-5+CHN-5+ADM-3。**actor_kind 隐性维度** (admin / system sweeper / plugin lifecycle / 未来类型) 全装单表, 名实相符 — BPP-8 #532 5 plugin lifecycle 事件入此表是合法的 (跟 ADM-2.1 admin actor 同精神). 索引: 跟随 admin_actions 原表 (RENAME 透明) — `idx_admin_actions_target_user_id_created_at` + `idx_admin_actions_actor_id_created_at` + `idx_admin_actions_archived_at WHERE archived_at IS NOT NULL` (sparse idx AL-7.1)。**reverse compat**: alias view `admin_actions` AS SELECT * FROM `audit_events` 留 deprecated 至 Phase 5+, 战马原 gorm `(AdminAction).TableName() == "admin_actions"` 0 改。 |
| `impersonation_grants` | `id` PK (UUID), `user_id` NOT NULL (FK users.id 逻辑, 业主自己 grant), `granted_at` Unix ms, `expires_at` Unix ms (granted_at + 24h server 固定), `revoked_at` Unix ms NULL (业主主动撤销 stamp; NULL 表示有效) | **Phase 4 / ADM-2.2 (`schema_migrations` v=23, PR #484)**。蓝图 `admin-model.md §1.4` 红线 2 第二档 ("Impersonate 必须显眼, 红色横幅 + 倒计时") + §3 数据模型片段 ("由 user 创建, admin 仅消费这条记录") + §4.1 R3 (ADM-1 文案 "24h 时窗顶部红色横幅常驻可随时撤销" 兑现锚)。立场 ⑦ impersonate 显眼 (业主授权 → 红横幅 + 24h 倒计时 + `[立即撤销]` 入口; admin 写动作需 grant 存在 + 未过期 + 未撤销, 否则 403 `impersonate.no_grant`)。**反约束**: 蓝图 §3 字面 "由 user 创建" — INSERT 路径仅 user-rail 进入, admin SPA 不开授予自己 impersonate 路径 (反向 grep `force_impersonate\|admin_impersonate_self` count==0); 期限固定 24h (server `expires_at = granted_at + 24h`, 不接受 client 传 expires_at); revoked_at 是唯一允许的 UPDATE 路径 (forward-only 立场 ⑤ 例外 — 撤销是业主主动权); 不挂 `actor_id` 列 (admin 不在此表); 不挂 `cursor` / `token` / `session_token` / `api_key` / `duration` 列。索引: `idx_impersonation_grants_user_id_expires` (ActiveGrant query 热路径)。 |
| `agent_status` | `agent_id` PK (1 row per agent), `state` NOT NULL CHECK ('busy','idle'), `last_task_id` (nullable, BPP frame 上行 task_id), `last_task_started_at` (nullable, Unix ms, busy 态填), `last_task_finished_at` (nullable, Unix ms, idle 态填), `created_at`, `updated_at` | **Phase 4 / AL-1b.1 (`schema_migrations` v=21, 本 PR)**。蓝图 `agent-lifecycle.md §2.3` 5-state busy/idle 真值 (2026-04-28 4 人 review #5 决议: source 必须 plugin 上行 `task_started` / `task_finished` frame, 没 BPP 不 stub)。立场 ① 拆三路径 — schema 仅 2 态 (busy/idle), API 层 (`GET /api/v1/agents/:id/status`) 5-state 合并 (error > busy > idle > online > offline), 跟 AL-3 presence_sessions + AL-4 agent_runtimes 拆死 (三表三路径互不污染)。**反约束**: 不挂 `is_online` / `presence` (AL-3 presence_sessions 拆死, agent task in-flight 但 hub 心跳超时是合法态); 不挂 `last_error_reason` / `endpoint_url` / `process_kind` (AL-4 agent_runtimes process-level 拆死, busy/idle 是 task-level); 不挂 `source` / `set_by` (立场 ② BPP 单源反人工伪造, server `PATCH /status` 405 admin god-mode 也 reject); 不挂 `cursor` 列 (跟 al_3_1 / al_4_1 同模式); 表无 ON DELETE CASCADE (蓝图 §2.3 保留状态历史 — agent 删后 status row 留账 admin 审计)。索引: `idx_agent_status_state` (busy 列表 lookup 热路径)。配套 server (PR #457 已 merged 进 #453 baseline): `IdleThreshold = 5 * time.Minute` reaper (acceptance §2.4 — 5min 无 frame 自动 idle); store helpers `SetAgentTaskStarted` / `SetAgentTaskFinished` / `ReapStaleBusyToIdle` (BPP-2 frame dispatcher 接入入口 stub). |
| `agent_state_log` | `id` PK AUTOINCREMENT (单调 history 序号), `agent_id` NOT NULL (FK users.id role='agent' 逻辑), `from_state` NOT NULL ('' / 'online' / 'busy' / 'idle' / 'error' / 'offline' 5 字面 union, AL-1a 三态 + AL-1b 5-state 复用), `to_state` NOT NULL (同上), `reason` NOT NULL DEFAULT '' (error 转移复用 AL-1a 6 字面 byte-identical), `task_id` NOT NULL DEFAULT '' (BPP-2.2 task lifecycle 触发记 task_id; presence 触发留空), `ts` Unix ms | **Phase 4 / AL-1.4 (`schema_migrations` v=25, AL-1 wrapper milestone PR)**。蓝图 `agent-lifecycle.md §2.3` (4 态 + cross-state transition lock + 故障可解释) 兑现 — wrapper milestone 跟 AL-1a (#249) + AL-1b (#453/#457/#462) + BPP-2.2 (#485) 同源, 此表是历史 audit 轨迹 + state machine validator (`store.ValidateTransition` server-side gate)。立场 ① forward-only audit log (跟 admin_actions ADM-2.1 立场 ⑤ 同精神 — 不挂 `updated_at`, 错误录入只能再写新行); 立场 ② state machine 单源 (`AppendAgentStateTransition` 是 server-side state set 唯一入口); 立场 ③ task-driven (BPP-2.2 触发 busy/idle 转移时 task_id 必填, presence 触发留空); 立场 ④ reason 复用 AL-1a 6 字面 byte-identical (改 = 改 8 处单测锁链同源 — #249 + #305 + #321 + #380 + #454 + #458 + #481 + 此)。**反约束**: 不挂 `cursor` (跟 al_3_1 / al_4_1 / cv_*_1 / dm_2_1 / cv_4_1 / chn_3_1 / al_2a_1 / adm_2_1 / adm_2_2 同模式 RT-1 envelope frame 路径不下沉); 不挂 `org_id` (派生 users.org_id); 不挂 `state` 单字段 (语义在 to_state, 双字段引诱 redundant write); 不挂 `owner_id` (派生 users.owner_id)。索引: `idx_agent_state_log_agent_id_ts` DESC (owner GET /api/v1/agents/:id/state-log 热路径)。endpoint: `GET /api/v1/agents/:id/state-log` owner-only (non-owner → 403, non-agent → 404, unauthenticated → 401)。 |
