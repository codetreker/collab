...
| `admin_actions` | `id` PK (UUID), `actor_id` NOT NULL (FK admins.id 逻辑), `target_user_id` NOT NULL (FK users.id 逻辑, 蓝图 §1.4 红线 1 受影响者必有), `action` TEXT NOT NULL CHECK 5 字面 byte-identical ('delete_channel','suspend_user','change_role','reset_password','start_impersonation'), `metadata` TEXT NOT NULL DEFAULT '' (JSON 上下文), `created_at` Unix ms | **Phase 4 / ADM-2.1 (`schema_migrations` v=22, PR #470)**。蓝图 `admin-model.md §1.4` "谁能看到什么" 四档分层 + §2 "Audit 100% 留痕" + §1.4 三红线 (受影响者必感知 + admin 之间互留痕)。立场 ① 每写必留痕 (admin SPA 任意写动作 INSERT, ADM-2.2 server 反向 grep `skip_audit\|noAudit\|bypassAudit` count==0); 立场 ⑤ forward-only audit (蓝图 §2 不变量 — 错误录入只能再写新行, 不挂 `updated_at`); 立场 ⑥ admin ∉ 业务路径 (actor_id FK admins 独立表, ADM-0 红线承袭, 反向 `actor_id.*users\b` count==0)。**反约束**: 不挂 `org_id` 列 (受影响者 org 通过 users.org_id 派生不冗余存); 不挂 `session_id` (impersonate 走单独 impersonation_grants 表 ADM-2.2 v1 落); 不挂 `cursor` (跟 al_3_1 / al_4_1 / cv_*_1 / dm_2_1 / cv_4_1 / chn_3_1 / al_2a_1 同模式 RT-1 envelope cursor 不下沉 schema)。action CHECK 严格 reject 同义词 (`remove_channel`/`ban_user`) + 大小写漂移 + 字典外值 + 空字符串 (15 反约束值 PASS)。索引: `idx_admin_actions_target_user_id_created_at` (受影响者 GET /api/v1/me/admin-actions 热路径) + `idx_admin_actions_actor_id_created_at` (admin SPA GET /admin-api/v1/audit-log 热路径)。 |
