...
| Reg ID | Source | Test path / grep | Owner | Trigger PR | Status |
|---|---|---|---|---|---|
| REG-DM2-001 | dm-2.md §1.0.a 表 schema 三轴 (PK + NOT NULL 全列断言) | `internal/migrations/dm_2_1_message_mentions_test.go::TestDM21_CreatesMessageMentionsTable` (TBD: flip via DM-2.2 PR) | 战马B / 烈马 | DM-2.1 #361 占号 / DM-2.2 翻 | ⚪ pending |
| REG-DM2-002 | dm-2.md §1.0.b UNIQUE(message_id, target_user_id) dedup | `dm_2_1_message_mentions_test.go::TestDM21_RejectsDuplicateMentionPerMessage` + `TestDM21_AllowsMultiTargetPerMessage` | 战马B / 烈马 | DM-2.1 #361 占号 / DM-2.2 翻 | ⚪ pending |
| REG-DM2-003 | dm-2.md §1.0.c idx_message_mentions_target_user_id (mention 路由热路径) | `dm_2_1_message_mentions_test.go::TestDM21_HasTargetUserIDIndex` | 战马B / 烈马 | DM-2.1 #361 占号 / DM-2.2 翻 | ⚪ pending |
| REG-DM2-004 | dm-2.md §1.0.e 反约束 column list (cursor / fanout_to_owner_id / cc_owner_id / owner_id / target_kind / read_at / acknowledged_at 全反向断言不存在) | `dm_2_1_message_mentions_test.go::TestDM21_NoCursorOrFanoutOwnerColumns` (7 列反向) | 战马B / 烈马 | DM-2.1 #361 占号 / DM-2.2 翻 | ⚪ pending |
| REG-DM2-005 | dm-2.md §1.0.d migration v=14→v=15 sequencing + idempotent (CV-2.1 抢 v=14 顺延; AL-4.1 后顺延 v=16) | `dm_2_1_message_mentions_test.go::TestDM21_Idempotent` + `TestDM21_MentionsPKMonotonic` + `registry.go` v=15 字面锁 | 战马B / 烈马 | DM-2.1 #361 占号 / DM-2.2 翻 | ⚪ pending |
| REG-DM2-006 | dm-2.md §2.1 在线 ping target → WS push only (owner sniff count==0) | `internal/api/dm_2_2_mention_test.go::TestDM22_OnlinePushOnly` (TBD) + `MentionPushedFrame` 8 字段 byte-identical | 战马 / 烈马 | DM-2.2 (待) | ⚪ pending |
| REG-DM2-007 | dm-2.md §2.2 离线 fallback owner system DM 文案锁 byte-identical `{agent_name} 当前离线，#{channel} 中有人 @ 了它，你可能需要处理` | `dm_2_2_mention_test.go::TestDM22_OfflineFallbackSystemDM` + `grep -nE '当前离线，#' packages/server-go/internal/api/messages.go` count≥1 | 战马 / 烈马 | DM-2.2 (待) | ⚪ pending |
| REG-DM2-008 | dm-2.md §2.3 节流 (agent, channel) 5min (clock fixture 第1次/+1min 不触发/+5min1ms 第2次) | `dm_2_2_mention_test.go::TestDM22_FallbackThrottle5Min` (TBD) | 战马 / 烈马 | DM-2.2 (待) | ⚪ pending |
| REG-DM2-009 | dm-2.md §2.4 反约束 — 离线 fallback 不转发原 body (隐私 §13 红线 反向 grep `system.*DM.*body` count==0) | `dm_2_2_mention_test.go::TestDM22_FallbackNoRawBody` (TBD) + grep | 飞马 / 烈马 | DM-2.2 (待) | ⚪ pending |
