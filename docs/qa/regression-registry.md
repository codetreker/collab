...
### AL-4 (acceptance template + stance + 文案锁 已锁, schema 实施全闭 #398, server/client 占号待 AL-4.2/4.3 真翻, 5 🟢 / 5 ⚪)
...
### DM-2 mention (PR #361 merged DM-2.1 schema v=15, 占号 10 ⚪ pending; 真翻 DM-2.2/2.3 实施 PR)
...
| REG-DM2-010 | dm-2.md §3.1 mention render `@{display_name}` 蓝色高亮 + 反向 DOM grep raw UUID 不漏 (data-mention-id attr 可有, 文本节点不可有) | `packages/e2e/tests/dm-2-3-mention.spec.ts` (TBD) + DOM `[data-mention-id]` count≥1 + 文本节点 raw UUID count==0 | 战马 / 烈马 | DM-2.3 (待) | ⚪ pending |
...
### CV-4 artifact iterate (PR #405 merged CV-4.1 schema v=18, 5 🟢; CV-4.2/4.3 待 merge, 5 ⚪ 占号)

> CV-4.1 schema-only PR (跟 #361 DM-2.1 / #410 CHN-3.1 占号模式同) — 实施 PR (#409 CV-4.2 server / TBD CV-4.3 client) 真翻 ⚪→🟢 时对号填 evidence.
