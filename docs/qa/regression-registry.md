...
| REG-AL3-008 | al-3.md §2.4 5s 节流 + 60s 心跳超时 (clock fixture) + §2.5 `presence.changed` frame 字段白名单 (无 last_heartbeat_at / connection_count / endpoints[]) — 留 AL-3.3 心跳重试逻辑 | TBD AL-3.3 PR | 战马A / 烈马 | TBD | ⚪ pending |
...
| REG-AL3-010 | al-3.md §3.1-§3.4 client UI dot DOM lock + agent-only + 跨 org + §4.1 admin god-mode 元数据白名单 + §4.2 跨 org 默认 false | TBD AL-3.3 PR + ADM-2 实施 | 战马A / 烈马 | TBD | ⚪ pending |
