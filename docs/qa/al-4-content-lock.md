...
| **error** (status: running → error) | `"{agent_name} 出错: {reason}"` (`{reason}` byte-identical 跟 `agent/state.go` 6 reason codes 一致, 跟 AL-3 #305 ③ error 文案同根) | agent owner DM only | ❌ `{reason}` 必须 ∈ {`api_key_invalid`, `quota_exceeded`, `network_unreachable`, `runtime_crashed`, `runtime_timeout`, `unknown`} (改 = 改三处: state.go + 表 CHECK + DM 文案); ❌ 不附 stack trace / log tail (隐私 + ADM-0 ⑦ admin 元数据 only 同精神) |
