...
| ④ | AL-1a #249 6 reason codes + #313 §1 AL-4.2 | **last_error_reason 复用 #249 6 reason codes byte-identical** — 不另造 runtime-only reason 枚举 | **是** `agent_runtimes.last_error_reason` ∈ {`api_key_invalid`, `quota_exceeded`, `network_unreachable`, `runtime_crashed`, `runtime_timeout`, `unknown`} 字面同 `agent/state.go` Reason*; client SPA error badge 跟 `lib/agent-state.ts` REASON_LABELS 同源 (跟 AL-3 #305 ③ error 文案同模式); **不是** 自造 runtime-specific reason (`startup_failed` / `port_in_use` 等 — 都映射到 `runtime_crashed` 或 `unknown`); **不是** 加 reason 枚举 (改 = 改三处: AL-1a state.go + AL-4 表 CHECK + AL-3 client REASON_LABELS) | v0: 6 reason 字面锁; v1 同 |
