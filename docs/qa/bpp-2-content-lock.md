...
> **关联**: spec `docs/implementation/modules/bpp-2-spec.md` (战马E v0) + acceptance `docs/qa/acceptance-templates/bpp-2.md` (战马E v0) + 复用 AL-1a #249 6 reason byte-identical (跟 AL-3 #305 + AL-4 #321 三处单测锁同源)
...
### ④ AL-1a 6 reason 字面 byte-identical 三处单测锁同源
...
跟 #249 (AL-1a 三态机) + AL-3 #305 (presence error 旁路) + AL-4 #321 (runtime status DM 文案) **三处单测锁同源** — 改 = 改四处 (现加 BPP-2.2 task_finished 是第四处). `internal/agent/state.go::Reason*` 常量是 source-of-truth.
...
| ⑤ | reason 字典承袭 AL-1a #249 6 项 (改 = 改三处+1) | concept-model.md §1.6 + agent-lifecycle.md §2.3 故障 UX | `internal/agent/state.go::Reason*` 6 常量是 source-of-truth, BPP-2.2 task_finished + AL-3 #305 + AL-4 #321 同源 |
...
| AL-1a ✅ #249 | 6 reason 字典字面 byte-identical | 改三处+1 (#249 + AL-3 #305 + AL-4 #321 + BPP-2.2) |
