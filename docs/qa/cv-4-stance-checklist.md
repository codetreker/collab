...
| ④ | spec #365 §1 + AL-1a #249 6 reason | **state 4 态机锁死 + reason 走 REASON_LABELS 三处单测锁** (改 reason = 改三处) | **是** state 转移图: `pending→running` (agent 接管) / `pending→failed` (timeout / runtime fail-closed) / `running→completed` (commit ok) / `running→failed` (runtime error); state CHECK reject 'unknown'; failed reason ∈ AL-1a 6 reason byte-identical (`api_key_invalid|quota_exceeded|network_unreachable|runtime_crashed|runtime_timeout|unknown`) **三处单测锁** (#249 + AL-3 #305 + 此立场); **不是** state 回退 (`completed→running reject` / `failed→pending reject`); **不是** failed 自动 retry (隐式 bypass owner 决策违反立场 ②); **不是** intent_text 模板 (留 v3+) | v0: 4 态机 + 6 reason; v1 同, runtime 真接管走 AL-4 落地后 |
...
- CV-4 entry 闸: 立场 ①-⑦ 全锚 + §2 黑名单 grep 全 0 (除标 ≥1) + reason 三处单测锁 (#249 + AL-3 #305 + 本 stance, 跟 AL-4 #379 v2 同源) + IterationStateChangedFrame 9 字段 byte-identical 跟 RT-1 共序
