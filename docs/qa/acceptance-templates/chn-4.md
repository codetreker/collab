...
| 验收项 | 实施方式 | Owner | 实施证据 |
|---|---|---|---|
| 1.1 `<ChannelView>` 加 tab switcher (`chat` / `workspace`) — 复用 CV-1.3 #346 ArtifactPanel 路径无新组件; tab state 走 URL `?tab=chat\|workspace` (deep-link 支持, 跟 CV-4.3 `?diff=` 同模式); 默认 `chat` | vitest + e2e | 战马 / 烈马 | `packages/client/src/__tests__/channel-view-tabs.test.ts::renders chat + workspace tab DOM byte-identical` (TBD); URL `?tab=workspace` 进入 workspace tab; `grep -rnE 'data-tab=["'"'"'](chat\|workspace)["'"'"']' packages/client/src/components/ChannelView.tsx` count≥2 |
| 1.2 反约束 DM 视图无 workspace tab — `if channel.type==='dm' return null` (CHN-2 #357 立场 ② byte-identical 跟 7 源同根: #354 ④ + #353 §3.1 + #357 ② + #364 + #371 + #374 + #378 ④) | vitest + e2e DOM 反断 | 战马 / 烈马 | `__tests__/channel-view-tabs.test.ts::DM type renders no workspace tab` (TBD); e2e `[data-kind="dm"] [data-tab="workspace"]` count==0; `grep -rnE 'data-kind=["'"'"']dm["'"'"'].*data-tab=["'"'"']workspace["'"'"']\|dm.*workspace.*tab' packages/client/src/components/` count==0 |
| 1.3 反约束 双 tab 不交叉 — chat tab 不渲染 ArtifactPanel / workspace tab 不渲染 MessageList; mention `@artifact` preview 走 CV-3 #370 ⑥ 独立路径 (md 80字 / code 5行+徽标 / image 192px) | vitest + grep | 飞马 / 烈马 | `__tests__/channel-view-tabs.test.ts::chat tab no ArtifactPanel + workspace no MessageList` (TBD, 双向反断); `grep -rnE 'data-tab=["'"'"']chat["'"'"'].*ArtifactPanel\|data-tab=["'"'"']workspace["'"'"'].*MessageList' packages/client/src/` count==0 |
...
| 验收项 | 实施方式 | Owner | 实施证据 |
|---|---|---|---|
| 2.1 `GET /api/v1/channels/:id` 返 body 加 `default_tab="chat"` 字段 byte-identical (server 端固定常量, 不入库 — 个人偏好覆盖留 CHN-3 user_channel_layout 但 v1 不开) | unit | 战马 / 烈马 | `internal/api/channels_test.go::TestCHN42_GetChannel_DefaultTabConstant` (TBD, response.default_tab == "chat" 字面 byte-identical assert) |
| 2.2 反约束 不动 channels 表 schema — `default_tab` 是 server 端固定常量返回, 不入库 | migration drift test | 飞马 / 烈马 | `chn_4_2_no_schema_change_test.go::TestCHN42_ChannelsTableUnchanged` (TBD, PRAGMA table_info(channels) 列字面跟 CHN-1 #276 一致, 无 default_tab 列); `grep -rnE 'ALTER TABLE channels.*ADD.*default_tab' packages/server-go/internal/migrations/` count==0 |
| 2.3 反约束 不开 `PUT /api/v1/channels/:id/default_tab` 作者级偏好 endpoint (留 v3+; v1 server 常量足够 demo 价值, CHN-3 立场 ② 锁 + 野马 #378 立场 ⑥) | grep | 飞马 / 烈马 | `grep -rnE 'PUT /api/v1/channels/.*/default_tab\|POST.*channel.*tab.*config' packages/server-go/internal/api/` count==0 |
| 2.4 反约束 不挂 user_channel_layout default_tab 列 (CHN-3 立场 ② "偏好仅 collapsed + position 两维"锁) | grep | 飞马 / 烈马 | `grep -rnE 'ALTER TABLE user_channel_layout.*ADD.*default_tab\|user_channel_layout.*default_tab' packages/server-go/internal/migrations/` count==0 |
...
| 验收项 | 实施方式 | Owner | 实施证据 |
|---|---|---|---|
| 3.1 e2e `chn-4-collab-skeleton.spec.ts` 全流链 — 创 channel (private kind) → invite agent (CHN-1 #286 既有) → switch chat tab 发 message + agent reply (DM-2 #372 路径) → switch workspace tab 创 markdown artifact (CV-1.2 #342) → 三 kind artifact 全演示 (markdown + code + image_link, 跟 CV-3 #363 + #370 byte-identical) → anchor + comment + resolve (CV-2.2 #360) | e2e | 战马 / 烈马 | `packages/e2e/tests/chn-4-collab-skeleton.spec.ts::全流链 ≤30s 闭环` (TBD); 跟 CV-1.3 #348 §3.3 真 4901+5174 同模式 |
| 3.2 立场 ③ 走真 server-go(4901) + vite(5174) 不 mock — runtime stub via direct owner commit 实施 e2e 显式注释标 `// CV-4 runtime stub: direct owner commit (not server mock)` 区分两层 (跟 #375 §1 + #378 立场 ③ 字面对应); 反向 grep server mock 0 hit | e2e + grep | 飞马 / 烈马 | `chn-4-collab-skeleton.spec.ts` 含注释字面 `// CV-4 runtime stub: direct owner commit (not server mock)` count≥1; `grep -rnE 'mock.*4901\|jest\.mock.*server-go\|fakeServer.*4901\|nock.*4901' packages/e2e/tests/chn-4*.spec.ts` count==0 |
...
| 3.4 反约束 不新起 WS frame — RT-1 4 frame 已锁 (ArtifactUpdated 7 / AnchorCommentAdded 10 / MentionPushed 8 / IterationStateChanged 9), CHN-4 不引入第 5 个; tab 切换是 client URL state 不上 server | grep | 飞马 / 烈马 | `grep -rnE 'NewChannelTabFrame\|ChannelTabChanged\|TabSwitchedFrame' packages/server-go/internal/ws/` count==0 |
| 3.5 G3.4 demo 双 tab 截屏归档 — `docs/qa/screenshots/g3.4-chn4-{chat,workspace}.png` Playwright `page.screenshot()` 入 git (反 PS 修改, CI 主动截屏); 跟 G2.4#5 / G2.5 / G2.6 demo 联签同模式 | Playwright | 战马 / 野马 / 烈马 | `chn-4-collab-skeleton.spec.ts::G3.4 双 tab 截屏归档` (TBD); 截屏文件 commit 入 docs/qa/screenshots/; `grep -rnE 'g3\.4-chn4-(chat\|workspace)\.png\|page\.screenshot.*g3\.4-chn4' packages/e2e/tests/chn-4*.spec.ts` count≥2 |
| 3.6 **G3.4 退出闸三签机制** — 战马 (e2e 真过 ≤3s 各 tab) + 烈马 (acceptance template 对齐) + **野马 (双 tab 截屏文案锁验: chat 🤖↔👤 二元 #347 byte-identical + workspace kind 三态 #370 ① `data-artifact-kind` byte-identical)** | 联签 | 战马 + 烈马 + 野马 | PR description 含三签字面 (跟 G2.4#5 / G2.5 / G2.6 demo 联签同模式); 缺一签 → 退出闸不通过 |
