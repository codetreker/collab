...
- REG-DL3-001 ⚪ EventsMetrics SSOT 4 helper (RowCount + TotalSize × channel + global) byte-identical + 阈值 const SSOT (ThresholdEventsRows=100k / ThresholdEventsBytes=100MB) 跨层锁 blueprint §5 字面同源
- REG-DL3-002 ⚪ EventsRetentionSweeper.Stats() 4 字段 (reaped_total / reaped_last_run / last_run_at / next_run_at) + 累计 reaped over time + ctx-aware deterministic (sync.WaitGroup + Done() chan)
- REG-DL3-003 ⚪ must-persist 4 类不计 reaped (DL-2 #615 MustPersistKindPrefixes byte-identical 跨 milestone const SSOT 锁链承袭)
- REG-DL3-004 ⚪ Prometheus 4 gauge + 1 counter byte-identical 跟蓝图 §5 metric name 同源 + structured log 4-dict event kind SSOT (反同义词 threshold reached/size limit/row limit body 0 hit)
- REG-DL3-005 ⚪ system DM 给 admin 走 ADM-2 #484 system DM 5 模板路径 byte-identical + 反 admin god-mode bypass (ADM-0 §1.3) + 反平行 EventsMetrics 实施
- REG-DL3-006 ⚪ 0 endpoint 行为改 + 0 schema (复用 DL-2 events 表) + 既有全包 unit + e2e + vitest 全绿不破 + post-#614 haystack gate 三轨过 + 跨十二 milestone const SSOT 锁链承袭 (BPP-2 + REFACTOR-REASONS + DM-9 + CHN-15 + AP-4-enum + DL-1 + REFACTOR-1 + REFACTOR-2 + NAMING-1 + DL-2 + DL-3)
