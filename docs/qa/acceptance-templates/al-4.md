...
| 2.5 error 回填 `status='error'` + `last_error_reason` 复用 AL-1a #249 6 reason 枚举字面 byte-identical (handshake_failed/auth_rejected/connection_refused/heartbeat_timeout/protocol_violation/internal_error) | unit | 战马 / 烈马 | _(待 AL-4.2 PR)_ — `runtimes_test.go::TestRuntimeErrorReasonsMatchAL1aEnum` (跟 #249 lib/agent-state.ts 6 reason 字面对) |
...
| 验收项 | 实施方式 | Owner | 实施证据 |
|---|---|---|---|
| 3.1 agent 详情页 `Runtime` 卡片显示 `endpoint_url` + `process_kind` + 4 态 status badge (registered/running/stopped/error) 颜色字面对齐 AL-1a #249 REASON_LABELS 模板 | e2e | 战马 / 烈马 | _(待 AL-4.3 PR)_ — `packages/e2e/tests/al-4-3-runtime-card.spec.ts::立场 ① 4 status badge 颜色 + DOM lock` |
| 3.2 start/stop 按钮 owner-only (非 owner DOM 不渲染槽位); 反约束 e2e: `[data-testid="runtime-start-btn"]` 在非 owner 视图 count==0 | e2e | 战马 / 烈马 | _(待 AL-4.3 PR)_ — `al-4-3-runtime-card.spec.ts::立场 ② owner-only btn 反向断言` |
| 3.3 error 状态显 reason label (走 client `lib/agent-state.ts` REASON_LABELS 同源 #249) + "查看日志" 直达入口 (蓝图 §2.3 故障可解释字面) | e2e | 战马 / 烈马 | _(待 AL-4.3 PR)_ — `al-4-3-runtime-card.spec.ts::立场 ③ error reason label + 日志入口跳转` |
