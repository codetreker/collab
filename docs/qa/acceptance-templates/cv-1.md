...
> 拆 PR: **CV-1.1** schema (`artifacts` + `artifact_versions` 表 + migration v=13) — ✅ #334 (cd7e12a) merged / **CV-1.2** server API (POST 创 + commit + rollback + WS push) — ✅ #342 (b2ed5c0) merged / **CV-1.3** client SPA canvas UI — TBD
> Owner: 战马A 实施 / 烈马 验收
