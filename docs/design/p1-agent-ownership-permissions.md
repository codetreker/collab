...
在现有 schema 基础上增加/修改以下表。所有变更通过一次迁移脚本 `migrations/002_p1_permissions.sql` 完成。
...
const code = crypto.randomBytes(10).toString('base32').toLowerCase(); // 16 chars
...
位置：`server/src/db/migrations/002_p1_permissions.sql` + 启动时执行器。
...
**幂等性保证**：
- `ALTER TABLE ADD COLUMN` 用迁移版本号表 `schema_migrations(version)` 记录，已执行跳过
- 所有 INSERT 用 `OR IGNORE`，依赖 `UNIQUE(user_id, permission, scope)`

**事务安全**：单个 BEGIN/COMMIT 包裹；失败回滚，版本号不写入。

**执行时机**：server 启动时 `runMigrations()`，所有版本号 < current 的文件顺序执行；已在 P0 做过一次（`001_initial.sql`），沿用机制。
...
| # | Task | 预估 | 依赖 |
|---|---|---|---|
| T1 | 迁移脚本 `002_p1_permissions.sql` + 启动执行器更新 | 3h | — |
