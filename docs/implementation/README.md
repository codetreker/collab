...
| agent_invitations 状态机 | `state` 列直接 TEXT enum + CHECK ('pending'/'accepted'/'declined'/'expired'); 状态机 helper (`store.AgentInvitation.Transition`) 只有 3 条边 (pending → 三终态), 终态无出边 | 若 v1 出现"重新邀请"或"再次发送"语义, 拆 lookup 表 (state_id INT FK → invitation_states) 并允许扩展状态; 当前 enum 直接落 string 在 v0 接受 | Phase 2 / CM-4.0 | TODO |
| ... | ... | ... | ... | ... |
