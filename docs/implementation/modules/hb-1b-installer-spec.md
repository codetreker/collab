...
派 **zhanma-e** (HB stack 全程主战熟手, HB-1+HB-2.0+HB-2 v0(C)+v0(D)). 飞马 review.

✅ **APPROVED with 3 必修**:
🟡 必修-1: 三平台真 build matrix CI 跑过 (跟 HB-2.0 #605 IPC matrix fail-fast: false 同精神)
🟡 必修-2: ed25519 manifest verify 真测 (反 silent skip), unit test 真测 invalid signature reject
🟡 必修-3: 4 grant_type 字面 byte-identical 跟 HB-3 #520 permission popup (反 hardcode 字面散落)

担忧 (1 项, 中度): notarization / code-sign 留账 — 真 release 前必补 (Apple Developer ID + EV cert), 但本 v1 PR 不阻塞 (走 self-signed dev-only 兜底).

| 2026-05-01 | 飞马 | v0 spec brief — HB-1B-INSTALLER 真 installer Go binary 三平台 (HB-1 #491 endpoint + HB-2 v0(D) #617 daemon 后续部署工具). 3 立场 + 3 段拆 + 10 反向 grep + 3 必修. 留账: notarization / code-sign / upgrade in-place / uninstall / iOS-Android (永不做) / silent install. zhanma-e 主战 (HB stack 全程熟手) + 飞马 ✅ APPROVED 3 必修. teamlead 唯一开 PR. |
