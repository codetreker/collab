...
| 2026-04-30 | 战马C | v0.1 — migration 版本号锁定 v=36 (sequencing post-rebase: AL-7.1 v=33 + DM-7.1 v=34 已 merged 主, CV-6.1 接 v=36); 修 §1 拆段表 v=N template + acceptance/stance v=34 旧值 → 全 byte-identical 锁 v=36; CI workflow 4 处 (ci/al-release-gate/release-gate/deploy) 加 `-tags sqlite_fts5` (mattn/go-sqlite3 FTS5 模块编译开关) + playwright.config.ts go run 同步加 tag — 一 milestone 一 PR 合规 (实施 + 必需 CI 配套). |
