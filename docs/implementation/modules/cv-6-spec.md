...
| 日期 | 作者 | 变化 |
|------|------|------|
| 2026-04-30 | 战马C | v0 spec brief — Phase 5+ wrapper milestone (跟 CV-2 v2 / CV-3 v2 / AL-5 / AP-2 / AP-3 同期, canvas-vision §1.4 续作 — artifact 全文搜索 给 owner 大量 artifact 时定位). 3 立场 (复用 SQLite FTS5 不另起 elasticsearch / search owner-only 跟 6 处 ACL 一致 / 反向 grep 不另起 search 表) + 5 反约束 grep + 3 段拆 (server schema FTS5 + endpoint / client SearchBox + result list / e2e+closure) + 4 件套 spec 第一件 (acceptance + stance + content-lock 后续, content-lock 不需要 server-only + minimal client DOM). 一 milestone 一 PR 协议默认 1 PR. |
| 2026-04-30 | 战马C | v0.1 — migration 版本号锁定 v=35 (sequencing post-rebase: AL-7.1 v=33 + DM-7.1 v=34 已 merged 主, CV-6.1 接 v=35); 修 §1 拆段表 v=N template + acceptance/stance v=34 旧值 → 全 byte-identical 锁 v=35; CI workflow 4 处 (ci/al-release-gate/release-gate/deploy) 加 `-tags sqlite_fts5` (mattn/go-sqlite3 FTS5 模块编译开关) + playwright.config.ts go run 同步加 tag — 一 milestone 一 PR 合规 (实施 + 必需 CI 配套). |
