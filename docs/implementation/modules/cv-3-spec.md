...
| 2026-04-29 | 飞马 | v1 — 准实施二次审 follow-up patch 修 3 处 drift 帮战马C 不踩坑: (1) 列名 `kind` → `type` 字面修齐 (CV-1.1 cv_1_1_artifacts.go:72 既有 `artifacts.type TEXT NOT NULL CHECK (type = 'markdown')` byte-identical, RT-1 frame.Kind json:"kind" 是 frame 字段独立约定不混 server schema 列名); (2) §1 CV-3.1 加 SQLite 12-step 重建路径明示 (CREATE artifacts_new + INSERT SELECT + DROP + RENAME + 重建 idx, transaction 包住 — SQLite 不支持 ALTER CHECK 直接改); (3) §1 CV-3.1 字面锁 `POST /artifacts` body 字段名 `type` (跟 createArtifactRequest.Type artifacts.go:208 byte-identical, 不引入 `kind` 字段名), §2 + §5 加 CV-1.2 立场 ④ 旧锁文案 `"type must be 'markdown' (v1)"` (artifacts.go:253) 实施时**删** + §3 grep 加反向断言 0 hit; §3 grep 反约束从 5 条 → 6 条; 战马C 实施 PR push 前先看本 v1 patch |
