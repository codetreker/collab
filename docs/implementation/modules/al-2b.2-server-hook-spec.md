...
| 日期 | 作者 | 变化 |
|---|---|---|
| 2026-04-29 | 烈马 | v0 — AL-2b.2 server hook spec lite (≤100 行 budget 实际 71 行紧凑), 跟 #465 spec brief 立场承袭 + cv-4.2 #409 push hook 同模式 + #452 acceptance v1 §2+§3 byte-identical 对齐. 0 部分: PATCH 成功 → fanout 5 步流程 (validate / UPDATE / push / 异步 ack / response). 1 部分: 单 PR 3 文件改 + 1 测试新 (~295 行 budget). 2 部分: 5 处 fail-closed (stale schema_version + cross-owner + runtime-only field + admin god-mode + idempotency_key). 3 部分: 5 test 跟派活字面 byte-identical (PushOnPatch / AckHandler / StaleSchemaVersionReject / CrossOwnerReject / RuntimeOnlyReject). 4 部分: 跟 cv-4.2 #409 push hook code 字面同模式锚. 5 部分: 反约束 (frame 字段不动 / ack/retry / 双轨 / batch). 等 #447 merge + AL-2b.1 frame 落 main 后真起 .2 实施 PR. |
| 2026-04-29 | 烈马 | v1 — sync patch 跟 #472 (AL-2b.1 BPP frame) + #481 真实落点对齐: §1 拆段 file paths drift 修齐 (3 处) — (a) `internal/ws/hub.go` 改 +0 → 改 +20 (跟 #472 真落点 frame 在 internal/bpp 不在 internal/ws, ws.Hub ↔ BPP envelope routing 需新桥) + (b) `internal/ws/agent_config_ack_dispatcher.go` → `internal/bpp/agent_config_ack_dispatcher.go` (跟 #472 frame 落点 internal/bpp/envelope.go 同包就近) + (c) `internal/api/al_2b_2_server_hook_test.go` → `internal/bpp/al_2b_2_server_hook_test.go` (跟 #472 al_2b_frames_test.go 落点同包). §2 fail-closed 5 处 + §3 测试设计 + §4 cv-4.2 push hook code mirror + §5 反约束 + §6 v0 字面跟 #452 acceptance v1 + #465 spec brief 立场承袭仍 byte-identical 不变. 跟 #338/#393/#467 docs only follow-up sync 同模式 — 一 milestone 一 PR 协议下不开 follow-up PR, 直接进 .worktrees/al-2b feat/al-2b 跟 #481 其他 commit 同 push (跟 #474/#479 字面对齐). |
