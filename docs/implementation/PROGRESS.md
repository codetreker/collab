...
- [x] **AL-9** audit log SSE push (admin live monitor) ✅ Phase 5+ wrapper milestone — 战马C 整 milestone 一 PR (4 件套 + 三段全闭): spec `al-9-spec.md` v0 649a704 (3 立场 admin-rail SSE only / audit fan-out 锁链终结 / 反 polling fallback + 5 反向 grep) + stance + acceptance + content-lock (SSE 3 状态文案 + AuditLogStream DOM data-* + 5 错码 toast 双向锁); AL-9.1 server SSE endpoint (`internal/api/audit_events.go` GET /admin-api/v1/audit-log/events admin-rail mw + subscribe-before-handshake ordering 跟 #533 fix 同模式 + 5 错码 const byte-identical + Last-Event-ID/?since= resume + 50 行 backfill limit + 15s heartbeat) + AuditEventFrame 7 字段 envelope byte-identical (`internal/ws/audit_event_frame.go`); AL-9.2 fan-out 锁链终结 (`Store.InsertAdminAction` 末尾调 `auditPusher.PushAuditEvent` nil-safe seam — 改一处, 6 audit writer 自动 fan-out ADM-2.1/AL-1/BPP-4/BPP-8/AP-2/AL-7); AL-9.3 client (`packages/client/src/admin/components/AuditLogStream.tsx` useEventSource + DOM 字面锁 byte-identical content-lock §2 + 反 polling fallback 立场 ③ + AUDIT_ERR_TOAST 5 错码 toast map + AUDIT_SSE_STATUS 3 文案); 7 vitest PASS + go test (ws 6 unit / store 3 unit / api 6 unit + reverse-grep 等价 0 hit) 全绿. 立场 3 项 + 跨链锁第 6 处 audit (HB-1/HB-2/BPP-4/BPP-8/HB-3 v2/AL-7/AL-9 5 字段 byte-identical) + envelope 7 字段同 ArtifactUpdated/IterationStateChanged/AnchorCommentAdded 模式 + cursor 共序跟 RT-1.1 同源. 不在范围: v2 user-rail audit / SSE filter / 离线追溯 unbounded / agent SSE / search / saved query / GC. REG-AL9-001..006 占号 (closure flip 待 PR merge).
- [ ] **ADM-3** 来源 C 混合
...
| 2026-04-30 | 战马C | AL-9 整 milestone 一 PR (Phase 5+ wrapper milestone — admin SPA audit log SSE live monitor; ADM-2 #484 续作 admin live monitor 真有缝 + Phase 5+ 同期 AL-7/CV-6/CV-3 v2/CV-2 v2/AP-2/AP-3): spec `al-9-spec.md` 69 行 (≤80) v0 649a704 (3 立场 admin-rail SSE only / audit fan-out 锁链终结 / 反 polling fallback + 5 反向 grep + 3 段拆) + stance `al-9-stance-checklist.md` (3 立场 + 7 边界含 ④ subscribe-before-handshake 跟 #533 fix 同模式) + acceptance `al-9.md` (1.1-1.6 / 2.1-2.4 / 3.1-3.5) + content-lock `al-9-content-lock.md` (admin SPA UI 必锁 — SSE 3 状态文案 + AuditLogStream DOM data-* + 5 错码 toast 双向锁). AL-9.1 server SSE endpoint (`internal/api/audit_events.go::handleAuditEvents` GET /admin-api/v1/audit-log/events admin-rail mw + subscribe-before-handshake ordering 跟 #533 fix 同模式 反 race window + 5 错码 const byte-identical + Last-Event-ID/?since= resume + 50 行 backfill limit 立场 ⑨ + 15s heartbeat) + AuditEventFrame 7 字段 byte-identical envelope (`internal/ws/audit_event_frame.go` 跟 ArtifactUpdated 7 / IterationStateChanged 9 / AnchorCommentAdded 10 同模式 + auditBuffer cap 200 + SnapshotAuditBuffer 立场 ⑨); AL-9.2 fan-out 锁链终结 (`internal/store/admin_actions.go::InsertAdminAction` 末尾调 `auditPusher.PushAuditEvent` nil-safe seam + `internal/store/db.go` AuditPusher interface + SetAuditPusher; `server.go` wire `s.store.SetAuditPusher(s.hub)` — 改 InsertAdminAction 一处, 6 audit writer 自动 fan-out ADM-2.1/AL-1/BPP-4/BPP-8/AP-2/AL-7); AL-9.3 client (`packages/client/src/admin/components/AuditLogStream.tsx` useEventSource + 50 行 cap + auto-scroll + DOM 字面锁 byte-identical content-lock §2 — data-testid="audit-log-stream"/data-state 三 enum/data-action 6-tuple + 反 polling fallback 立场 ③ + AUDIT_ERR_TOAST 5 错码 toast map; `packages/client/src/admin/api.ts` AUDIT_SSE_STATUS 3 文案 byte-identical + AuditEventFrame interface; `pages/AdminAuditLogPage.tsx` mount above poll table); 7 vitest PASS (DOM contract / 3 文案 / state transitions / row data-* / admin-rail URL only / 50 cap FIFO / AUDIT_ERR_TOAST 5 keys exact) + go test ws 6 unit (cursor monotonic 32 racer / buffer cap 200 / since filter / nil-safe ctor / FrameType byte-identical) + store 3 unit (insert triggers push / nil pusher safe / 5 字段 byte-identical) + api 6 unit (admin-rail Content-Type / user-rail not mounted / user cookie 401 / cursor invalid 400 / fan-out integration / reverse-grep equivalent 3 pattern 0 hit) 全绿. 立场 3 项 + 跨链锁第 6 处 audit (HB-1/HB-2/BPP-4/BPP-8/HB-3 v2/AL-7/AL-9 5 字段 byte-identical actor/action/target/when/scope) + envelope 7 字段同 ArtifactUpdated 模式 + cursor 共序跟 RT-1.1 同源 + 5 错码 const 跟 AP-1/AP-2/AP-3/CV-2 v2/CV-3 v2/CV-6 同模式 + ADM-0 §1.3 admin god-mode 不入业务路径同精神. 不在范围: v2 user-rail audit / SSE filter / 离线追溯 unbounded / agent SSE / search / saved query / GC. REG-AL9-001..006 6 🟢; 总计 382→388 / active 356→362 / pending 26 不变. |
