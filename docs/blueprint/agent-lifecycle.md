...
| 状态 | 含义 | 触发 |
|------|------|------|
| **在线（online）** | runtime 已连接，等待消息 | plugin 在 WS / poll 心跳 |
| **工作中（busy）** | 正在处理一个任务 | runtime 报告活动 / message 待 ack |
| **空闲（idle）** | 在线但长时间无任务（>5min） | 心跳但无活动 |
| **故障（error）** | runtime 报错或失联 | API key 失效、超限、网络断、进程崩溃 |

#### 故障态的关键设计

故障态**必须可解释**——不能只是红点。UI 要展示：

- **原因码**：`api_key_invalid` / `quota_exceeded` / `network_unreachable` / `runtime_crashed` / `unknown`
