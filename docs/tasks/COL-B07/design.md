...
| 同 Agent 同名 | upsert 覆盖 | 后注册覆盖先注册，DB `UNIQUE(agent_id, name)` 约束 |
...
否决理由：Agent 需暴露公网端口，增加网络复杂度（超时/重试/认证），与现有 WS/SSE 通道重复。消息通道方案零额外成本。

### 全局唯一命令名 + Agent 前缀（已否决）

`/agentA:search` 格式。否决理由：用户需记前缀语法，学习成本高。按 Agent 分组显示更直观。
