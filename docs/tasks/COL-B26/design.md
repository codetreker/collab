...
| D7 | 新频道默认位置 | 所属分组末尾（position = 最大值 + 1 的 lexorank） | — | 符合 PRD |
| D8 | 未分组频道展示 | group_id = NULL，显示在所有分组上方 | — | 符合 PRD |

## 4. DB Schema

### 4.1 channels 表变更（ALTER）
...
2. 查找 after_id 的 position 和下一项的 position
3. 计算中间 lexorank
4. UPDATE channels SET position = ?, group_id = ? WHERE id = ?
5. 广播 WS 事件
...
复用现有 modal 组件。入口：sidebar 顶部 `[+]` 下拉菜单增加"创建分组"选项。
...
| T2 | DB migration：channels 加 position 列 + channel_groups 表 | 1h |
...
| T16 | channels 加 group_id FK 列（migration） | 0.5h |
...
| T29 | 现有频道 migration — 为已有频道生成初始 position（按 created_at 排列） | 1h |
...
| 并发拖拽冲突 | 两个 owner 同时移动频道到相同位置 | SQLite WAL + busy_timeout 保证写串行化；各 owner 只操作自己的频道，实际冲突概率极低 |
