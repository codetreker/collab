...
### 0. 铁律: teamlead 阻塞操作派 subagent 做
**血账**: teamlead 跑 `sleep 60`, `gh run view --log`, 长 grep, `git rebase`, `playwright test` 等阻塞命令时, 主线程被冻住, idle agent 全员等死, 派活停滞.

任何 ≥10s 阻塞操作 (sleep / 长 CI log fetch / test run / large grep / rebase) **必须**走 Agent tool subagent 异步:
- `subagent_type: general-purpose` + `run_in_background: true`
- subagent 完成自动通知, teamlead 主线继续派活

**例外**: <5s 命令 (gh pr view JSON / 短 grep / git status) 直接 Bash OK.

判别: 如果命令可能让你"等"超过 10 秒, 派 subagent. teamlead 协调中心不停。
