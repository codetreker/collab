...
func TestBPPEnvelopeFrameWhitelist(t *testing.T) {
	...
	if got, want := len(envs), 13; got != want {
		t.Fatalf("BPP envelope count: got %d, want %d (BPP-1 control 6 + data 3 + AL-2b ack +1 + BPP-2.2 task +2 + BPP-3.1 permission_denied +1 = 13)", got, want)
	}
	wl := bpp.BPPEnvelopeWhitelist()
	if got, want := len(wl), 13; got != want {
