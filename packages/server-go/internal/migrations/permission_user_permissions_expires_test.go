...
// TestAP_AddsExpiresAtColumn pins acceptance §1.1 — schema 加列 nullable
// (NULL = 永久, 跟蓝图 §1.2 字面承袭). 反向: 不挂 NOT NULL, 现网行
// expires_at 全 NULL = 行为零变.
func TestAP_AddsExpiresAtColumn(t *testing.T) {
	t.Parallel()
	db := openMem(t)
	runAP11(t, db)
...
// TestAP_HasSparseIndex pins acceptance §1.2 — partial index 覆盖
// expires_at IS NOT NULL 行 (sweeper 热路径 v2+ 业务化时挂).
func TestAP_HasSparseIndex(t *testing.T) {
	t.Parallel()
	db := openMem(t)
	runAP11(t, db)
...
// TestAP_NullExpiresIsLegit pins acceptance §1.3 — NULL expires_at 是
// 合法终态 (永久, 现网行为不变).
func TestAP_NullExpiresIsLegit(t *testing.T) {
	t.Parallel()
	db := openMem(t)
	runAP11(t, db)
...
// TestAP_AcceptsExplicitExpires pins acceptance §1.3 — schema 接受
// expires_at 显式赋值 (v2+ 业务化时 server 端写, v1 留 schema slot).
func TestAP_AcceptsExplicitExpires(t *testing.T) {
	t.Parallel()
	db := openMem(t)
	runAP11(t, db)
...
// TestAP_RegistryHasV24 pins v=24 sequencing — registry.go 字面锁,
// AL-1b.1 v=21 / ADM-2.1 v=22 / ADM-2.2 v=23 / **AP-1.1 v=24**.
func TestAP_RegistryHasV24(t *testing.T) {
	t.Parallel()
	for _, m := range All {
		if m.Version == 24 {
