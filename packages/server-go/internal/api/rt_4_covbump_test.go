...
// REG-RT4-cov-bump — opportunistic cov bump for channels.go uncovered handlers.
func TestRT4_CovBump_ListChannelGroups(t *testing.T) {
	t.Parallel()
...
func TestRT4_CovBump_ListGroups_AfterCreate(t *testing.T) {
	t.Parallel()
	ts, _, _ := testutil.NewTestServer(t)
	ownerToken := testutil.LoginAs(t, ts.URL, "owner@test.com", "password123")
	resp, _ := testutil.JSON(t, http.MethodPost, ts.URL+"/api/v1/channel-groups", ownerToken,
		map[string]any{"name": "rt4-cov-grp"})
