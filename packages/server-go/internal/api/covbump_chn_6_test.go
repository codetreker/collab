...
// REG-CHN6-cov-bump v5 — AL-5 recover error branches.
func TestCHN_6_CovBump_AL5RecoverErrors(t *testing.T) {
	t.Parallel()
	ts, _, _ := testutil.NewTestServer(t)
	ownerToken := testutil.LoginAs(t, ts.URL, "owner@test.com", "password123")
...
// REG-CHN6-cov-bump v5 — sanitizeImpersonateGrant nil branch via JSON GET.
func TestCHN_6_CovBump_ImpersonateGrantSanitizeNil(t *testing.T) {
	t.Parallel()
	ts, _, _ := testutil.NewTestServer(t)
	ownerToken := testutil.LoginAs(t, ts.URL, "owner@test.com", "password123")
...
// REG-CHN6-cov-bump v5 — host-grants validation + lifecycle.
func TestCHN_6_CovBump_HostGrantsLifecycle(t *testing.T) {
	t.Parallel()
	ts, _, _ := testutil.NewTestServer(t)
	ownerToken := testutil.LoginAs(t, ts.URL, "owner@test.com", "password123")
