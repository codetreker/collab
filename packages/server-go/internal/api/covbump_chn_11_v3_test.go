...
// REG-CHN_11-cov-bump v3 — AL-8 audit-log filter param branches.
func TestCHN_11_CovBump_v3_AuditLogFilters(t *testing.T) {
	t.Parallel()
...
// REG-CHN_11-cov-bump v3 — impersonation-grant lifecycle (create/get/revoke).
func TestCHN_11_CovBump_v3_ImpersonateGrantLifecycle(t *testing.T) {
	t.Parallel()
	ts, _, _ := testutil.NewTestServer(t)
...
// REG-CHN_11-cov-bump v3 — AL-7 audit-retention/override branches.
func TestCHN_11_CovBump_v3_AL7AuditRetentionOverride(t *testing.T) {
	t.Parallel()
	ts, _, _ := testutil.NewTestServer(t)
	adminToken := testutil.LoginAsAdmin(t, ts.URL)
