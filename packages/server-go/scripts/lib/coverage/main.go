...
func parseConfig() Config {
	c := Config{
		...
		RaceDetection:    false,
		ExcludePackages:  "borgee-server/scripts/,borgee-server/cmd,borgee-server/internal/testutil,borgee-server/internal/api/cm5stance,borgee-server/internal/testutil/regression_suite",
		ExcludeFiles:     "internal/testutil/,main.go",
		ExcludeFuncs:     "",
...
func runTests() ([]PackageResult, map[string]int, map[string]int, int) {
	...
	if cfg.CoverPkg != "" {
		args = append(args, "-coverpkg="+cfg.CoverPkg)
	}
...
func generateHTMLReport() {
	cmd := exec.Command("go", "tool", "cover", "-html="+cfg.CoverProfile, "-o", "./.coverage/test_coverage.html")
